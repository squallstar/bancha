...
Class Content {
	...
	public function rebuild()
	{
		...
		foreach ($filenames as $filename)
		{
			...
			$node = simplexml_load_file($this->xml_folder . $filename);

			//Filename sanitize
			$safe_filename = str_replace(' ', '_', str_replace('.xml', '', $filename));

			//Adds the type in the array
			$all_types[] = $safe_filename;
			...
			//Contiene tutti i tipi per le query in fondo
			$all_types_id[] = $type_id;
			...
			foreach ($node->fieldset as $fieldset_node)
			{
				...
				$fieldset_name = isset($fieldset_node->name) ? (string)$fieldset_node->name : 'Senza titolo';
				...
				foreach ($fieldset_node->field as $field)
				{
					...
					if (in_array($field_name, $restricted_names))
					{
	...
	function records_from_sql_xml($sql, $id_type = '')
	{
		...
		$this->CI->db->from((string)$sql->from);
		...
		if (isset($sql->type))
		{
			$id_tipo = is_numeric((string)$sql->type) ? (string)$sql->type : $this->type_id((string)$sql->type);
